...
for in_file_name in glob.glob(in_file_pat):
    ...
    file_num = os.path.splitext(file_name)[0].split('-')[-1]
    if int(file_num) < start_num:
        continue
        
    out_file_name = os.path.join(out_file_path,"success_"+file_name)
    with bz2.open(in_file_name, mode="rt", encoding="UTF-8") as infile, \
         bz2.open(out_file_name,mode='wt',  encoding="UTF-8") as outfile:
        for i,line in enumerate(infile):
            if i%1000 == 0:
